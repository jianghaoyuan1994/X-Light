...
# from onpolicy.envs.sumo_files.config import config
# from onpolicy.envs.sumo_files_new.config import config
...
class R_Actor_SUMO(nn.Module):
    ...
    def __init__(self, args, obs_space, action_space, device=torch.device("cpu")):
        ...
        self._policy_head = ActorModel(114, 8)
    ...
    def evaluate_actions(self, obs, action, policy_head_input, masks, available_actions=None, active_masks=None):
        ...
        # hidden_layer, logits = self.body_model(obs, available_actions)
        # hidden_layer = self.body_model(obs, available_actions)
        # hidden_states = self.pre(hidden_layer).sum(-1).squeeze(1)
        ...
        # dist_entropy = action_logits.entropy().unsqueeze(-1)
        if active_masks is not None:
...
class R_Critic_SUMO(nn.Module):
    def __init__(self, hidden_size, device):
        ...
        self.tpdv = dict(dtype=torch.float32, device=device)
        self._value_head = CriticModel(114)
