...
import numpy as np
import torch
from onpolicy.runner.shared.base_runner import Runner, SUMOBaseRunner
import wandb
import imageio
...
class SUMORunner(SUMOBaseRunner):
    ...
    def run(self):
        ...
        for episode in range(self.begin_ep, episodes):
            ...
            for step in range(self.episode_length):
                print('-----step', step)
                # Sample actions
                values, actions, action_log_probs, hidden_layer = self.collect(step, deterministic)
                    
                # Obser reward and next obs
                ###### tsc specified actions
                obs, rewards, dones, infos = self.envs.step(actions.astype(np.int64))
                
                self.ava = self.envs.get_unava_phase_index()
                available_actions = self.get_ava_actions(self.ava)

                data = obs, rewards, dones, infos, values, actions[:,:,None].astype(np.int64), action_log_probs[:,:,None], hidden_layer, available_actions
            ...
            if deterministic:
                tmp = 0
                for info in infos:
                    tmp += np.average(get_reward(info)) / len(infos)
                print(tmp)
                    ...
                    if self.env_name == "SUMO":
                        if not self.all_args.cotrain:
                            ...
                            for i in range(len(infos)):
                                env_infos[i] = {}
                                info = infos[i]
                                for agent_id in range(self.all_args.map_agents[i]):
                                    for k, v in info[list(info.keys())[agent_id]].items():
                                        if k not in env_infos[i]:
                                            env_infos[i][k] = []
                                        env_infos[i][k].append(v) 
                            self.log_env(env_infos, total_num_steps)        
    ...
    def collect(self, step, deterministic):
        ...
        value, action, action_log_prob, hidden_layer \
            = self.trainer.policy.get_actions(np.concatenate(self.buffer.share_obs[step],axis = 1),
                            np.concatenate(self.buffer.obs[step],axis = 1),
        ...
        # hidden_states = np.array(np.split(_t2n(hidden_states), self.n_rollout_threads))
        hidden_layer = np.array(np.split(_t2n(hidden_layer), self.n_rollout_threads))
    ...
    def insert(self, data):
        ...
        self.buffer.insert(share_obs, obs, actions, action_log_probs, values, rewards, masks, hidden_layer=hidden_layer, available_actions=available_actions)
