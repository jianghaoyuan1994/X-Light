...
class SubprocVecEnv(ShareVecEnv):
    def __init__(self, env_fns, cotrain=False, spaces=None):
        """
        envs: list of gym environments to run in subprocesses
        """
        self.waiting = False
        ...
        if cotrain:
            ...
            for i in range(len(env_fns)):
                ...
                observation_space_, share_observation_space_, action_space_ = self.remotes[i].recv()
                observation_space += observation_space_
                share_observation_space += share_observation_space_
                action_space += action_space_
                self.map_agents.append(len(observation_space_))
            ShareVecEnv.__init__(self, 1, observation_space,
                                share_observation_space, action_space)
        else:
            self.remotes[0].send(('get_spaces', None))
            observation_space, share_observation_space, action_space = self.remotes[0].recv()
    ...
    def step_async(self, actions):
        if self.cotrain:
            ...
            start = 0
            for i in range(len(self.map_agents)):
                end = start + self.map_agents[i]
                self.remotes[i].send(('step', actions[start:end]))
                start = end    
        else:
            for remote, action in zip(self.remotes, actions):
                remote.send(('step', action))
    ...
    def step_wait(self):
        ...
        obs, rews, dones, infos = zip(*results)
        if self.cotrain:
            return np.expand_dims(np.concatenate(obs, axis = 1), axis = 0), np.expand_dims(np.concatenate(rews), axis = 0), np.expand_dims(np.concatenate(dones), axis = 0), infos
        else:
            return np.stack(obs), np.stack(rews), np.stack(dones), infos
    ...
    def reset(self):
        ...
        obs = [remote.recv() for remote in self.remotes]
        if self.cotrain:
            return np.expand_dims(np.concatenate(obs, axis = 1), axis = 0)
        else:
            return np.stack(obs)
    ...
    def reset_task(self):
        for remote in self.remotes:
            remote.send(('reset_task', None))
        if self.cotrain:
            return np.expand_dims(np.concatenate([remote.recv() for remote in self.remotes], axis = 1), axis = 0)
        else:
            return np.stack([remote.recv() for remote in self.remotes])
